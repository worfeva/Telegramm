...
async def log_message(update: Update):
        ...
        cursor_logs.execute("INSERT INTO logs (message, date) VALUES (?, ?)", 
...
def backup_db():
        ...
        shutil.copyfile(REVIEWS_DB_FILE, os.path.join(BACKUP_DIR, f"reviews_{timestamp}.db"))
def delete_review_and_traces(review_id, context=None):
    ...
    try:
        ...
        cur_logs_local.execute("DELETE FROM logs WHERE message = ? OR message = ?", (text_r, title))
...
async def read_reviews(update: Update, context: ContextTypes.DEFAULT_TYPE):
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT id, title, rating, nickname FROM reviews WHERE approved=1 ORDER BY created_at DESC")
    ...
    keyboard = [[InlineKeyboardButton(f"{r[1]} ({'⭐'*r[2]}) — {r[3]}", callback_data=f"user_read_{r[0]}")] for r in reviews]
    await update.message.reply_text("📖 Выберите отзыв для прочтения:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def user_read_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    title, rating, nickname, text_r = cursor.fetchone()
    conn.close()
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "read" in data:
        ...
        await secret_moderation(update, context)
...
read_reviews_handler = MessageHandler(filters.Regex("(?i)^отзывы$"), read_reviews)
moderation_handler = MessageHandler(filters.Regex(f"^{SECRET_MODERATION_CODE}$"), secret_moderation)
