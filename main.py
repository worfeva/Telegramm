...
async def start_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"👋 Добро пожаловать в систему отзывов об оказанных консультациях проекта «Ревматолог на связи»! Благодарим Вас за обратную связь. Это позволяет нам работать над нашими ошибками и улучшать качество услуг.\n\n"
...
async def read_reviews(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not reviews:
        text = "Пока нет одобренных отзывов."
        if message:
    ...
    text = "📖 Выберите отзыв для прочтения:"
    if message:
